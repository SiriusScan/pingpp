...
func (c *Collector) RunResult(ctx context.Context, in engine.CollectorInput) (engine.CollectorResult, error) {
	if in.Endpoint == nil {
		return engine.CollectorResult{}, fmt.Errorf("vnc: endpoint required")
	}
	ref := in.Endpoint.Ref()
	...
	if err != nil {
		...
		return engine.CollectorResult{Outcome: engine.OutcomeFromError(err), Protocol: "vnc", Observations: []model.ObservationRecord{obs}}, nil
	}
	...
	if err == nil && n >= 12 && strings.HasPrefix(string(buf), "RFB ") {
		...
		return engine.CollectorResult{Outcome: engine.OutcomeSuccess, Protocol: "vnc", Observations: []model.ObservationRecord{obs}}, nil
	}
	obs.Completeness = "none"
	if err != nil {
		obs.Error = err.Error()
	...
	_ = obs.SetPayload(payload)
	return engine.CollectorResult{Outcome: engine.OutcomeNoMatch, Protocol: "vnc", Observations: []model.ObservationRecord{obs}}, nil
}
